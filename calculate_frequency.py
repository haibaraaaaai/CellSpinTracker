...
def process_frame(frame, roi_size=64):
    ...
    if np.linalg.norm(
        np.array(leftmost) - np.array(center)) > np.linalg.norm(
            np.array(rightmost) - np.array(center)):
        farthest_point = leftmost
    else:
        farthest_point = rightmost

    return selected_contour, farthest_point
...
def calculate_frequency(
    ...
    while cap.isOpened():
        ...
        contour, farthest_point = process_frame(frame, roi_size)
    ...
    while cap.isOpened():
        ...
        contour, farthest_point = process_frame(frame, roi_size)
        if farthest_point is not None:
            direction = calculate_freq(prev_point, farthest_point)
